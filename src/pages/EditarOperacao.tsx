...
const EditarOperacao = () => {
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        try {
            await supabase.from('registro_operacoes').update({ op: selectedOp, data, hora_inicial: horaInicial, hora_final: horaFinal, observacao, navio_id: selectedNavio || null }).eq('id', operacaoId);
            
            await supabase.from('equipamentos').delete().eq('registro_operacoes_id', operacaoId);
            let equipamentosParaSalvar: any[] = [];
            if (selectedOp === 'NAVIO') { equipamentosParaSalvar = equipamentosNavio.filter(eq => eq.tag).map(({ id, ...rest }) => ({ ...rest, registro_operacoes_id: operacaoId, local: 'NAVIO', grupo_operacao: 'Operação Navio' })); }
            else { operacaoGrupos.forEach(g => { const eG = g.equipamentos.filter(eq => eq.tag).map(({ id, ...rest }) => ({ ...rest, registro_operacoes_id: operacaoId, local: selectedOp, grupo_operacao: g.nome })); equipamentosParaSalvar.push(...eG); }); }
            if (equipamentosParaSalvar.length > 0) { await supabase.from('equipamentos').insert(equipamentosParaSalvar); }

            await supabase.from('ajudantes').delete().eq('registro_operacoes_id', operacaoId);
            if (ajudantes.length > 0) { const ajudantesData = ajudantes.map(({ id, ...rest }) => ({ ...rest, registro_operacoes_id: operacaoId, data, local: selectedOp })); await supabase.from('ajudantes').insert(ajudantesData); }

            await supabase.from('ausencias').delete().eq('registro_operacoes_id', operacaoId);
            if (ausencias.length > 0) { const ausenciasData = ausencias.map(({ id, ...rest }) => ({ ...rest, registro_operacoes_id: operacaoId, data, local: selectedOp })); await supabase.from('ausencias').insert(ausenciasData); }
        ...
        <div className="min-h-screen pb-10" style={{ background: 'var(--gradient-primary)' }}>
            ...
            <form onSubmit={handleUpdate} className="p-4 space-y-4">
                <Card><CardHeader><CardTitle>Dados da Operação</CardTitle></CardHeader><CardContent className="space-y-4 pt-4">
                    ...
                    <div><Label>Observação do Turno</Label><Textarea value={observacao} onChange={(e) => setObservacao(e.target.value)} /></div>
                </CardContent></Card>
                
                {selectedOp === 'NAVIO' && (
                    <Card>
                        <CardHeader><CardTitle>Equipamentos do Navio</CardTitle></CardHeader>
                        <CardContent><div className="space-y-2">{equipamentosNavio.map(eq => (<div key={eq.id} className="flex items-center gap-2"><Input placeholder="TAG" value={eq.tag} onChange={e => updateEquipamentoNavio(eq.id, 'tag', e.target.value)} /><Input placeholder="OPERADOR/MOTORISTA" value={eq.motorista_operador} onChange={e => updateEquipamentoNavio(eq.id, 'motorista_operador', e.target.value)} /><Button variant="ghost" size="icon" onClick={() => removeEquipamentoNavio(eq.id)}><X className="h-4 w-4 text-red-500" /></Button></div>))}</div><div className="flex gap-2 mt-4"><Button type="button" onClick={() => addEquipamentoNavio(1)} className="flex-1">+1 Equipamento</Button><Button type="button" onClick={() => addEquipamentoNavio(10)} variant="secondary" className="flex-1">+10 Equipamentos</Button></div></CardContent>
                    </Card>
                )}
                {['HYDRO', 'ALBRAS', 'SANTOS BRASIL'].includes(selectedOp) && (
                    <div className="space-y-4">
                        {operacaoGrupos.map(grupo => (<Card key={grupo.id}><CardHeader><div className="flex items-center justify-between"><Input className="text-lg font-bold border-0 shadow-none focus-visible:ring-0 p-0" value={grupo.nome} onChange={e => updateOperacaoGrupo(grupo.id, e.target.value)} /><Button variant="ghost" size="icon" onClick={() => removeOperacaoGrupo(grupo.id)}><X className="h-5 w-5 text-red-500" /></Button></div></CardHeader><CardContent><div className="space-y-2">{grupo.equipamentos.map(eq => (<div key={eq.id} className="flex items-center gap-2"><Input placeholder="TAG" value={eq.tag} onChange={e => updateEquipamentoGrupo(grupo.id, eq.id, 'tag', e.target.value)} /><Input placeholder="OPERADOR" value={eq.motorista_operador} onChange={e => updateEquipamentoGrupo(grupo.id, eq.id, 'motorista_operador', e.target.value)} /><Button variant="ghost" size="icon" onClick={() => removeEquipamentoGrupo(grupo.id, eq.id)}><X className="h-4 w-4 text-red-500" /></Button></div>))}</div><div className="flex gap-2 mt-4"><Button type="button" onClick={() => addEquipamentoGrupo(grupo.id, 1)} size="sm" className="flex-1">+1 Equipamento</Button><Button type="button" onClick={() => addEquipamentoGrupo(grupo.id, 10)} size="sm" variant="secondary" className="flex-1">+10 Equipamentos</Button></div></CardContent></Card>))}
                        <Button type="button" onClick={addOperacaoGrupo} variant="outline" className="w-full">Adicionar Operação (Frente de Serviço)</Button>
                    </div>
                )}

                <Card>
                    <CardHeader><div className="flex justify-between items-center"><CardTitle>Ajudantes</CardTitle><Button type="button" onClick={addAjudante} size="sm"><Plus className="h-4 w-4 mr-2" />Ajudante</Button></div></CardHeader>
                    <CardContent className="space-y-4 pt-4">{ajudantes.length === 0 && <p className="text-center text-muted-foreground">Nenhum ajudante adicionado.</p>}{ajudantes.map(ajudante => (<Card key={ajudante.id} className="p-4 relative"><Button type="button" onClick={() => removeAjudante(ajudante.id)} size="icon" variant="ghost" className="absolute top-1 right-1 h-7 w-7"><X className="h-4 w-4 text-red-500" /></Button><div className="space-y-2"><div><Label>Nome</Label><Input value={ajudante.nome} onChange={(e) => updateAjudante(ajudante.id, 'nome', e.target.value)} /></div><div className="grid grid-cols-2 gap-2"><div><Label>Início</Label><Input type="time" value={ajudante.hora_inicial} onChange={(e) => updateAjudante(ajudante.id, 'hora_inicial', e.target.value)} /></div><div><Label>Fim</Label><Input type="time" value={ajudante.hora_final} onChange={(e) => updateAjudante(ajudante.id, 'hora_final', e.target.value)} /></div></div><div><Label>Observação</Label><Input value={ajudante.observacao} onChange={(e) => updateAjudante(ajudante.id, 'observacao', e.target.value)} /></div></div></Card>))}</CardContent>
                </Card>

                <Card>
                    <CardHeader><div className="flex justify-between items-center"><CardTitle>Ausências</CardTitle><Button type="button" onClick={addAusencia} size="sm"><Plus className="h-4 w-4 mr-2" />Ausência</Button></div></CardHeader>
                    <CardContent className="space-y-4 pt-4">{ausencias.length === 0 && <p className="text-center text-muted-foreground">Nenhuma ausência adicionada.</p>}{ausencias.map(ausencia => (<Card key={ausencia.id} className="p-4 relative"><Button type="button" onClick={() => removeAusencia(ausencia.id)} size="icon" variant="ghost" className="absolute top-1 right-1 h-7 w-7"><X className="h-4 w-4 text-red-500" /></Button><div className="space-y-2"><div><Label>Nome</Label><Input value={ausencia.nome} onChange={(e) => updateAusencia(ausencia.id, 'nome', e.target.value)} /></div><div><Label>Observação</Label><Input value={ausencia.obs} onChange={(e) => updateAusencia(ausencia.id, 'obs', e.target.value)} /></div><div className="flex items-center space-x-2"><Checkbox checked={ausencia.justificado} onCheckedChange={(checked) => updateAusencia(ausencia.id, 'justificado', !!checked)} /><Label>Justificado</Label></div></div></Card>))}</CardContent>
                </Card>
