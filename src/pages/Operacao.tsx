...
const Operacao = () => {
    ...
    const handleSaveStep1 = async () => {
        if (!user || !selectedOp || !data || !horaInicial) { 
            toast({ title: "Erro", description: "Preencha os campos de operação, data e hora inicial.", variant: "destructive" }); 
            return; 
        }
        setIsSaving(true);
        try {
            ...
            if (selectedOp === 'NAVIO') { 
                equipamentosParaSalvar = equipamentosNavio.filter(eq => eq.tag).map(eq => ({ 
            ...
            else if (['HYDRO', 'ALBRAS', 'SANTOS BRASIL'].includes(selectedOp)) { 
                operacaoGrupos.forEach(g => { 
                    const eG = g.equipamentos.filter(eq => eq.tag).map(eq => ({ 
                        registro_operacoes_id: operacao.id, 
                        local: selectedOp, 
                        tag: eq.tag.toUpperCase(), 
                        motorista_operador: eq.motorista_operador.toUpperCase(), 
                        grupo_operacao: g.nome.toUpperCase() 
                    })); 
                    equipamentosParaSalvar.push(...eG); 
                }); 
            }
            if (equipamentosParaSalvar.length > 0) { 
                const { error } = await supabase.from('equipamentos').insert(equipamentosParaSalvar); 
                if (error) throw error; 
            }
            ...
            console.error(error); 
            toast({ title: "Erro ao salvar Passo 1", variant: "destructive" }); 
        }
        finally { 
            setIsSaving(false); 
        }
    ...
    const handleSaveStep2 = async () => {
        ...
        setIsSaving(true);
        try {
            const ajudantesData = ajudantes.map(a => ({ 
                registro_operacoes_id: newOperacaoId, 
            ...
            console.error(error); 
            toast({ title: "Erro ao salvar Passo 2", variant: "destructive" }); 
        }
        finally { 
            setIsSaving(false); 
        }
    ...
    const handleSaveStep3 = async () => {
        if (!newOperacaoId || ausencias.length === 0) { 
        ...
        setIsSaving(true);
        try {
            const ausenciasData = ausencias.map(a => ({ 
                registro_operacoes_id: newOperacaoId, 
                nome: a.nome.toUpperCase(), 
                data, 
            ...
            console.error(error); 
            toast({ title: "Erro ao salvar Passo 3", variant: "destructive" }); 
        }
        finally { 
            setIsSaving(false); 
        }
            ...
            <div className="px-4 space-y-4">
                            ...
                            <>
                                <Card className="shadow-[var(--shadow-card)]">
                                    ...
                                    <CardContent className="space-y-4">
                                        ...
                                        <div>
                                            <Label htmlFor="observacao-turno">Observação do Turno (Opcional)</Label>
                                            <Textarea 
                                                id="observacao-turno" 
                                                value={observacao} 
                                                onChange={(e) => setObservacao(e.target.value.toUpperCase())} 
                                                placeholder="Alguma observação geral sobre a operação ou o turno..." 
                                            />
                                        </div>
                                    </CardContent>
                                </Card>
                                    ...
                                    <Card className="shadow-[var(--shadow-card)]">
                                        ...
                                        <CardContent>
                                            ...
                                            <div className="flex gap-2 mt-4">
                                                <Button onClick={() => addEquipamentoNavio(1)} className="flex-1">+1 Equipamento</Button>
                                                <Button onClick={() => addEquipamentoNavio(10)} variant="secondary" className="flex-1">+10 Equipamentos</Button>
                                            </div>
                                        </CardContent>
                                    </Card>
                                )}
                                ...
                                {['HYDRO', 'ALBRAS', 'SANTOS BRASIL'].includes(selectedOp) && (
                                    <div className="space-y-4">
                                        {operacaoGrupos.map(grupo => (
                                            <Card key={grupo.id} className="shadow-[var(--shadow-card)]">
                                                <CardHeader>
                                                ...
                                                <CardContent>
                                                    ...
                                                    <div className="flex gap-2 mt-4">
                                                        <Button onClick={() => addEquipamentoGrupo(grupo.id, 1)} size="sm" className="flex-1">+1 Equipamento</Button>
                                                        <Button onClick={() => addEquipamentoGrupo(grupo.id, 10)} size="sm" variant="secondary" className="flex-1">+10 Equipamentos</Button>
                                                    </div>
                                                </CardContent>
                                            </Card>
                                        ))}
                                ...
                                <Button onClick={handleSaveStep1} disabled={isSaving} className="w-full h-12 bg-accent hover:bg-accent/90 text-white font-semibold">
                                    {isSaving ? 'Salvando...' : 'Próximo: Ajudantes'}
                                </Button>
                            </>
                        )}
                    </>
                ...
                {step === 2 && (
                    <>
                        <Card className="shadow-[var(--shadow-card)]">
                            <CardHeader>
                                <div className="flex justify-between items-center">
                                    <CardTitle>Adicionar Ajudantes</CardTitle>
                                    <Button onClick={addAjudante} size="sm">
                            ...
                            <CardContent className="space-y-4 pt-4">
                                ...
                                {ajudantes.map(ajudante => (
                                    <Card key={ajudante.id} className="p-4 relative">
                                        <Button onClick={() => removeAjudante(ajudante.id)} size="icon" variant="ghost" className="absolute top-1 right-1 h-7 w-7">
                                            <X className="h-4 w-4 text-red-500" />
                                        </Button>
                                        <div className="space-y-2">
                                            <div>
                        ...
                        <Button onClick={handleSaveStep2} disabled={isSaving} className="w-full h-12 bg-accent hover:bg-accent/90 text-white font-semibold">
                            {isSaving ? 'Salvando...' : 'Próximo: Ausências'}
                        </Button>
                ...
                {step === 3 && (
                    <>
                        <Card className="shadow-[var(--shadow-card)]">
                            <CardHeader>
                                <div className="flex justify-between items-center">
                                    <CardTitle>Adicionar Ausências</CardTitle>
                                    <Button onClick={addAusencia} size="sm">
                            ...
                            <CardContent className="space-y-4 pt-4">
                                ...
                                {ausencias.map(ausencia => (
                                    <Card key={ausencia.id} className="p-4 relative">
                                        <Button onClick={() => removeAusencia(ausencia.id)} size="icon" variant="ghost" className="absolute top-1 right-1 h-7 w-7">
                                            <X className="h-4 w-4 text-red-500" />
                                        </Button>
                                        <div className="space-y-2">
                                            <div>
                        ...
                        <Button onClick={handleSaveStep3} disabled={isSaving} className="w-full h-12 bg-accent hover:bg-accent/90 text-white font-semibold">
